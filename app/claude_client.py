...
{{
  ...
  "owner_message": "<社長向け1-2文。中学生でも分かる日常語のみ。専門用語禁止（粗利率→『仕入れを引いた利益率』、EBITDA→『本業のもうけ』、運転資本→『日々の運営にいるお金』、自己資本比率→『自分のお金で経営してる割合』等に置換）。数字は『〇〇万円』『〇〇％』レベルで。例：『売上は順調に伸びてますが、商品が売れるスピードが落ちていて、倉庫に在庫がたまり始めています。このままだと現金が足りなくなる可能性があります。』>",
  "owner_what_to_do": "<社長向け1-2文。今すぐやることを命令形＋日常語で。例：『来月までに在庫の半分を値引き販売してください。これで現金が約500万円戻ってきます。』>",
...
【社長向け表現の重要ルール（北村先生FB対応）】
owner_message と owner_what_to_do は、決算書を読めない社長に1秒で伝わる表現にすること：
- 専門用語禁止：粗利率→「仕入れを引いた利益率」、営業利益→「本業のもうけ」、EBITDA→「本業の現金生成力」、運転資本→「日々の運営に必要なお金」、自己資本比率→「自分のお金で経営してる割合」、流動比率→「短期の支払い余力」、CCC→「現金が戻ってくるサイクル」、ROI→「投資に対する戻り」、KPI→「重要な数字」
- 数字は「〇〇万円増えた」「〇〇％下がった」など、増減と方向だけ伝える
- ストーリー優先：「だから何」「何が起きてる」「どうすればいい」の3点
- 怖がらせる表現は避ける（「危険」「破綻」より「気を付けたい」「もったいない」）
- 命令調はOKだが上から目線は避ける
