...
def generate_owner_pdf_content(result: dict, fd, client_name: str, period: str) -> dict:
    ...
    prompt = f"""あなたは中小企業の社長に「分析ベースの経営報告書」をプレゼンする税理士です。
このレポートは **社長が一人で読んで理解できる、ロジカルで証拠ベースの分析レポート** です。
税理士は説明なしで送付するため、文書自体が完結している必要があります。

【分析結果（要約済み）】
{_json.dumps(distilled, ensure_ascii=False, indent=2)}

【書き方の絶対ルール】
1. **ロジカル & 分析ベース**: 「数字 → だから何 → どうすべきか」の因果関係を明示
   - 例（OK）: 「現預金が3年で1.3億→6,600万円と半減しました。これは月190万円の借入返済が、本業の月間黒字3.8万円を遥かに上回っているためです。」
   - 例（NG: 感覚的）: 「現金が減っていて、少し心配な状況です。」
2. **専門用語は社長語に**: 営業利益→「本業のもうけ」、有利子負債→「借入の残り」、自己資本比率→「自分のお金で経営してる割合」、CCC→「現金が戻るサイクル」
3. **数字は丸めて1〜2桁**: 26,288万円→「2.6億円」、6,569万円→「約6,600万円」、45万円→「45万円」
4. **不安煽り禁止**: 「危険」「破綻」「底をつく」→ 「このペースが続くと〜年で底に達する計算」「気を付けたい点」
5. **断定/問いかけのバランス**: 事実は断定、意思決定は社長に委ねる
   - 事実: 「売上の77%が1社依存です」
   - 意思決定: 「依存度を下げる方向性で社長のご判断はいかがでしょうか」
6. **ストーリー優先 + データの裏付け**: 各セクションで必ず数字を1つ以上引用
7. **論理整合**: 「依存度77%」と言いつつ「その顧客から追加受注」のような矛盾NG
8. **「次の一歩」は具体的に**: 「採用を強化」より「年内に営業1名追加（年収400万）で年間2,000-3,000万円の上乗せを試算」

【出力JSON】
{{
  "cover_subtitle": "<表紙サブタイトル30字以内。会社の状況を分析的に一言で。例：『黒字化を達成、次は現金体質の強化が経営課題』>",
  "narrative_intro": "<2-3文 / 200-300字。今期の決算ハイライトを数字で。挨拶は最小限>",
  "narrative_situation": "<4-6文 / 500-700字。今の財務状況を **数字+因果+示唆** の構造で。良い点と懸念点を順に。専門用語は社長語に置換>",
  "key_numbers": [
    {{"label": "<指標名（社長語）>", "value": "<丸めた数字>", "comment": "<1文の分析的含意（業界比較や前期比を含めるとベター）>"}}
  ],
  "narrative_strengths": "<3-4文 / 300-400字。同業比較で誇れる点を **業界中央値との差分** で示す>",
  "narrative_issues": "<4-6文 / 500-700字。3つの課題を **数字 → 因果 → 影響予測** の構造で>",
  "narrative_proposal": "<4-6文 / 500-700字。打ち手3方向を提示。**やる前/やった後 の金額差分** を含める>",
  "narrative_outlook": "<3-4文 / 300-400字。打ち手を実行した場合の3年後・5年後の財務シナリオを数字で>",
  "next_actions": ["<1文の具体アクション。**期限・金額・主体** を含めるとベター。5個以内>"],
  "closing": "<2-3文 / 150-250字。締めの挨拶。「ご不明点があればお気軽にお問い合わせください」のニュアンス>"
}}

件数目安: key_numbers 4-6個、next_actions 3-5個。
