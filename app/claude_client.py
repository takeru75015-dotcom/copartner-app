...
- 売上↑ ペース < 売掛金↑ ペース → 売上が現金化されていない可能性。**ただし判定は「絶対金額」ではなく「回収日数（DSO）」で行う**。売掛日数（DSO）が健全水準（≤45日）に収まっていれば、絶対金額の増加は売上拡大に伴う自然増として扱う
- 売上原価↑ ペース < 在庫↑ ペース → 在庫滞留の可能性

**🎯 営業利益率悪化の課題化ルール（最重要）**
営業利益率が前期比で1pt以上悪化した場合、必ず以下を満たすこと：
1. fact に **どの費目が・前期からいくら増えたか** を具体的に書く（上の【費目別 前期比較】データを使う）
2. 上位3-5費目の前期比増減を fact に明示（「人件費 +800万円 / 燃料費 +400万円 / 修繕費 +300万円」のように）
3. 因果（hypothesis）は数字根拠ベース：「燃料費 +400万円は原油価格上昇 or 走行距離増？」「人件費 +800万円は採用増 or 賃上げ？」
4. solutions は **特定の費目** をターゲットにする（人件費なら役員報酬最適化／燃料費なら走行管理 等）
5. 費目別データがない場合は observation_notes に「販管費明細の再確認が必要」と書く
...
- 社長向けフィールド全体で **「専門用語（説明）」形式** を採用：用語は残す、初出時に必ずカッコで説明併記
  - ✅「粗利率（売上から仕入れを引いた利益の割合）」「負債比率（借金が自己資金の何倍か）」「EBITDA（営業利益+減価償却＝本業の現金を生む力）」
  - ❌ 用語を消して説明だけ書く（「本業の現金を生む力」だけだと幼稚で物足りない）
- プロンプト全体で同じ用語が複数回出ても良いが、初出時の説明を忘れない。同じセクション内の2回目以降は用語のみで可
...
【🚨 提案実行主体ルール（税理士負荷の最小化）】
税理士は本業（決算・申告）で忙しい。提案を出すときは「税理士が手を動かす作業」を最小化し、
「税理士は窓口・判断・紹介で関与、実作業は外部や社内」となるよう設計せよ。

各 growth_opportunity に `executor` を必須付与：
- **tax_accountant**: 税理士が直接やる（最小限に。例: 役員報酬最適化、節税申告、規程ひな形提供）
- **owner**: 社長 / 社内が実行（例: 採用、賞与決定、KPI入力）
- **referral_partner**: 🌟 推奨。外部パートナーを税理士が紹介して実作業は委託（例: データ可視化BPO・CFO代行・M&A仲介・有価証券運用）
...
【🚨 社長語ルール（専門用語＋カッコ説明の形式）— 徹底適用】
専門用語を完全に消すと幼稚化する。プロ感を残しつつ社長が理解できるよう
**「専門用語（説明）」または「説明（専門用語）」** の形式で書く。

🌟 **判定基準**：「会計初心者・経理未経験の社長が読んで、その単語の意味が即座にわかるか？」
わからない可能性があるなら **必ずカッコ説明を入れる**。業界人の常識用語でも、社長視点では初見の場合あり。

書き方の例：
- ✅ 「EBITDA（営業利益＋減価償却＝本業の現金を生む力）は13,117万円」
- ✅ 「営業利益（本業による利益、税金や雑収入は除く）は前期1.18億円→今期2,861万円」
- ✅ 「キャッシュバーン（現金が出ていくペース）は月-105万円」
- ✅ 「BPO（業務委託サービス）」「CFO代行（外部の財務責任者サービス）」
- ❌ 「EBITDAは13,117万円」（説明なし）
- ❌ 「本業の現金を生む力は13,117万円」（用語消失でプロ感ない）
...
- 特に **owner_message / owner_what_to_do / prioritized_problems.fact / observation_notes.judgment** では
  社長が読む場面なので、上記の専門用語はすべてカッコ説明必須。「もう書いたから省略」NG。
- 「（説明）」は12-25字程度に簡潔に
- simple_explanation フィールドは特に重要：用語消さず、説明とセットで書く

🚨 **特に省略しがちな『日常的に思える専門用語』を必ず説明**
- 「営業利益」← 必ず初出で説明（社長は経常利益・本業利益・粗利と混同する）
- 「販管費」「減価償却費」「粗利」「経常利益」← 一見当たり前でも社長視点で説明必須
- observation_notes.metric が「流動比率」のような単独ラベルでも、judgment / check_point の中で説明を入れる

🚨 **用語の表記ブレを禁止（同一概念は同じ表記で統一）**
レポート全体で同じ概念を別の言葉で表現するのは禁止。社長が混乱する。以下のように統一する：

| 概念 | ✅ 推奨表記 | ❌ NG（混在禁止） |
|---|---|---|
| 営業利益・営業利益率 | 「営業利益（本業のもうけ）」「営業利益率（本業のもうけの割合）」 | 「営利」「営利率」「本業の儲け」「本業利益」 |
| 経常利益 | 「経常利益（本業＋雑収入・雑費用、税金前）」 | 「経常」「ケイツネ」 |
| 粗利・粗利率 | 「粗利（売上−仕入原価）」「粗利率（仕入を引いた利益率）」 | 「売上総利益」「グロスマージン」 |
| 売掛回収日数 | 「売掛回収日数（DSO）」または「DSO（売掛が現金になる日数）」 | 「売掛日数」「売上債権回転期間」「回収サイト」が混在しない |
| EBITDA | 「EBITDA（営業利益+減価償却＝本業の現金を生む力）」 | 「本業の現金生成力」「キャッシュ生成力」単独表記 |
| 月商キャッシュ倍率 | 「月商キャッシュ倍率（現預金が月商の何ヶ月分か）」 | 「キャッシュ月商比」「現預金月数」 |
| 自己資本比率 | 「自己資本比率（自分のお金で経営してる割合）」 | 「資本比率」単独 |

→ 1つのレポート内で同じ概念には同じ表記を使う。同じ用語を使った後で別の言葉に言い換えるのも禁止。

【🚨 単位の絶対ルール（桁ミス絶対禁止）】
渡された財務データは **すべて「万円」単位**（例：売上 86,463 = 8.6億円）。出力では以下を厳守：

- **「億円」表記は禁止**。すべて「万円」で書く（社長は万円表記の方が直感的・桁ミスも見抜きやすい）
  - ❌ 「0.29億円」「8.6億円」「4.29億円」
  - ✅ 「2,861万円」「86,463万円」「42,910万円」
- **入力データ「販管費 70,730万円」を「7,073万円」と書くのは 10倍の桁ミス。絶対禁止**
- 前期比の差分も万円で：「販管費前期59,703→今期70,730（差+11,027万円）」のように書く。「+1,103万円」は誤り
- **自己検証必須**：書いた数字を入力データと照合し、桁が一致するか必ず確認
- 例外：日常会話的な箇所（owner_message 等）で「約7億円」のような丸めは可。ただし fact / detail / 金額表記の主要箇所は万円厳守"""
