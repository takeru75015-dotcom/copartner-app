...
【🚨 余剰キャッシュ活用ルール（必須提案・最優先）】
余剰が **3,000万円超** の場合、growth_opportunities の **rank=1 として** 「余剰キャッシュ◯◯万円の配分シナリオ」を必ず1件出すこと。
これは個別打ち手ではなく、**全体配分のロードマップ**として書く（社長が「結局どれくらい何に振ればいいの？」が一目で分かる形）。

【配分シナリオの必須フォーマット】
- title: 「余剰キャッシュ XX万円の3案比較（保守 / 標準 / 攻め）」
- rationale: なぜこの会社に配分検討が必要か（現金が積み上がり続けている／投資期の続き／規模拡大判断 等）
- fact: 余剰額の数式とその根拠（「現預金XX - 必要運転資金XX - 借入1年返済XX = 余剰XX」）
- impact_basis または concrete_calculation に **3案の配分内訳** を明記:
  例（余剰4,343万円のケース）:
   ・**保守案**: 借入返済前倒し 2,000万 + 社員還元 1,000万 + 手元維持 1,343万
     → 金利負担▲60万/年、社員定着率UP、リスクゼロ
   ・**標準案**: 設備投資（即時償却）2,400万 + 社員還元 900万 + 手元維持 1,043万
     → 法人税▲792万、社員定着率UP、投資効果検証可能
   ・**攻め案**: 小型M&A 3,000万（補助金▲800万で実質2,200万）+ 社員還元 500万 + 手元維持 1,643万
     → 売上+5,000万/年、PMI 6-12ヶ月、回収期間6-8年
- actions:
   - 「次回月次会議で3案を社長と比較・意思決定」
   - 「選んだ案の最初の1歩を1ヶ月以内に着手」

社長は「何に使うか決めかねている」状態が多いので、上記の **3案比較の俯瞰提案を rank=1** に置き、
個別の打ち手（M&A単品、設備投資単品、賞与単品）は rank=2以降に置く。
