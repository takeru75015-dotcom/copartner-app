...
【絶対ルール】
1. **専門用語は社長語に**: 営業利益→本業のもうけ、有利子負債→借入の残り、自己資本比率→自分のお金で経営してる割合、CCC→現金が戻るサイクル、流動比率→短期の支払い余力
...
{{
  ...
  "section6_conclusion": {{
    ...
    "closing_message": "<1-2文 / 100-150字。締めの挨拶。「ご不明点があればお気軽にお問い合わせください」風>"
  }}
}}

件数目安: section3 issues = 3件、section4 各 issue の solutions = 3件、section5 proposals = 2-4件（黒字時のみ）。
