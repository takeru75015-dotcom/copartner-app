...
def analyze_financials(fd: FinancialData, client_name: str, industry: str,
    ...
    if _skip_annual_trend:
        ...
        # period 昇順で並べる（期中は除外して年次のみで trend を作る）
        sorted_hd = sorted(
            [h for h in historical_data if getattr(h, "period_type", "") != "partial_aggregated"],
    ...
    if reference_books:
        ...
        _added = 0
        _total = 0
        _MAX_PER_BOOK = 3000      # 1冊あたりの上限
        _MAX_TOTAL = 12000        # 全書籍合計の上限（プロンプト肥大・コスト/コンテキスト超過を防ぐ）
