...
def _diagnose_company_state(fd, ebitda: dict, burn: dict, wc: dict) -> dict:
    ...
    # ★ 期中モード: 規模判定のため revenue を年率換算（×12/n）
    # n月累計で「年商1億未満」と誤判定して打ち手を抑制しないように。
    # 表示用の revenue は元のまま維持（呼び出し側で別途扱う）
    _period_type = getattr(fd, "period_type", "") or ""
    if _period_type == "partial_aggregated":
        _n = max(getattr(fd, "_partial_n_months", 0) or 1, 1)
        revenue = revenue * (12.0 / _n)  # 年率換算後の revenue で規模判定
...
def _compute_idle_cash(fd, burn: dict, wc: dict, industry: str = "") -> dict:
    ...
    cash = getattr(fd, "cash", 0) or 0
    revenue = getattr(fd, "revenue", 0) or 0
    # ★ 期中（partial_aggregated）の場合 revenue は n ヶ月累計なので ÷n で月商化
    _period_type = getattr(fd, "period_type", "") or ""
    if _period_type == "partial_aggregated":
        _rev_divisor = max(getattr(fd, "_partial_n_months", 0) or 1, 1)
    else:
        _rev_divisor = 12
    monthly_revenue = revenue / _rev_divisor if revenue else 0
    ...
    # 月次費用ベース（キャッシュランウェイ）= 営業を回すのに必要な実額/月
    # ★ 期中分析（partial_aggregated）の場合は ÷n（nヶ月分）で月額化
    # 年次（annual / annual_aggregated）は ÷12
    ...
    if _period_type == "partial_aggregated":
        ...
        _divisor = 12
    monthly_opex = (cost_of_sales + selling_expenses) / _divisor if (cost_of_sales + selling_expenses) > 0 else 0
    # 月次費用が取れない場合、売上を _divisor で割って暫定的に推定（fallback）
    if monthly_opex == 0:
        _rev = getattr(fd, "revenue", 0) or 0
        if _rev > 0:
            monthly_opex = _rev / _divisor * 0.95  # 売上の95%程度を費用と仮置き（保守的）
    idle_months = round(idle / monthly_opex, 1) if monthly_opex > 0 else 0
    ...
    return {
        ...
        "idle_cash_label": (
            "潤沢（次の戦略投資の予算として活用必須・寝かせ過ぎ）" if idle_months >= 3
            else "中程度（節税×小型投資の余地あり）" if idle_months >= 1
...
- 月次費用（売上原価+販管費）/12: {idle_info['monthly_opex']:,}万円/月
- ★ 余剰キャッシュ: {idle_info['idle_cash']:,}万円（{idle_info['idle_cash_label']}）
- ★ 余剰の月数: **月次費用の {idle_info['idle_months']} ヶ月分**（キャッシュランウェイ換算）

🚨 余剰キャッシュの扱い方（スタンス指示・必ず読め）:
- 余剰キャッシュがあること自体は「課題」ではない（健全な状態）。
  → **prioritized_problems には絶対に書くな**（Rule 4 健全水準は課題化禁止に抵触する）
- ただし「機会損失」としては存在する。寝かせている＝取れた打ち手を取っていない状態。

【月次費用ベースの判定基準】
  ・**3ヶ月分超 → 明らかに余剰**。growth_opportunities に配分シナリオを必ず1件出す
  ・**1〜3ヶ月分 → やや余剰**。observation_notes 記録 + 余裕あれば軽い提案
  ・**1ヶ月未満 → 適正範囲**。触れなくてよい

【3ヶ月分超で出す場合の構造】
  → observation_notes に1件記録：
     metric="現預金/月次費用比" / current_value="{idle_info['idle_months']}ヶ月分" /
     judgment="月次費用の3ヶ月分超で寝かせ過ぎ" /
     check_point="活用方針を社長と相談する余地"
  → growth_opportunities に「機会」として1件提案：
     ・3案配分シナリオ（保守/標準/攻め）を concrete_calculation に書く
     ・addresses="現状機会: 余剰キャッシュ活用" を必ずセット
     ・rank は自由。他にもっと重要な課題・機会があればそちらを優先してよい

【💡 余剰キャッシュ活用：3案配分シナリオ（適切な場面で出す）】
余剰が **3,000万円超** の場合、「全体配分の俯瞰提案」を growth_opportunities に1件含めると有益。
ただし「必須」ではなく、当該会社にとって最も価値が高い場合のみ rank=1〜3 に置くこと。

【配分シナリオを出す場合のフォーマット】
- title: 「余剰キャッシュ XX万円の3案比較（保守 / 標準 / 攻め）」
- fact: 余剰額の数式（「現預金XX - 必要運転資金XX - 借入1年返済XX = 余剰XX」）
- concrete_calculation に **3案の配分内訳** を明記:
  例（余剰4,343万円のケース）:
   ・**保守案**: 借入返済前倒し 2,000万 + 社員還元 1,000万 + 手元維持 1,343万
   ・**標準案**: 設備投資（即時償却）2,400万 + 社員還元 900万 + 手元維持 1,043万
   ・**攻め案**: 小型M&A 3,000万（補助金▲800万で実質2,200万）+ 社員500万 + 手元1,643万
- actions: 次回月次会議で3案を比較・意思決定 / 選んだ案の最初の1歩を1ヶ月以内に着手
...
# 💤 寝ている資産（固定化された資金）の検知
# ---------------------------------------------------------------------------
# 保険積立金・長期/短期貸付金・投資有価証券・出資金など「現金化 or 回収 or 活用の
# 余地がある資金」を決算書内訳から機械的に抽出する。AI任せ（非決定的）にせず Python で
# 必ず検知し、「金額が大きく状況に合う場合のみ」提案させる（無理な課題化はしない）。
_DORMANT_ASSET_RULES = [
    # (表示名, マッチキーワード, 論点/check_point)
    ("保険積立金", ["保険積立"],
     "解約返戻金額と保障目的（役員退職金原資等）を確認。遊休なら次の投資原資 or 資金繰りクッションに"),
    ("長期貸付金", ["長期貸付金", "長期貸付"],
     "貸付先（役員・関係会社の可能性）と回収見込みを確認。役員貸付なら認定利息など税務論点あり"),
    ("短期貸付金", ["短期貸付金", "短期貸付"],
     "貸付先と回収予定を確認。回収困難なら貸倒引当金の検討"),
    ("投資有価証券", ["投資有価証券"],
     "保有目的・含み損益・流動性を確認。政策保有なら現金化余地"),
    ("有価証券", ["有価証券"],
     "保有目的・含み損益・流動性を確認"),
    ("出資金", ["出資金"],
     "出資先の実態と回収可能性を確認"),
...
def _compute_dormant_assets(fd) -> dict:
    """決算書内訳から寝ている資産（固定化資金）を抽出する。"""
    try:
    ...
    found = {}  # 表示名 -> {"amount", "check_point"}
    for section in ("fixed_assets_detail", "current_assets_detail"):
        d = bd.get(section) or {}
        if not isinstance(d, dict):
            continue
        for raw_key, val in d.items():
            if raw_key.startswith("__") or not isinstance(val, (int, float)) or val <= 0:
                continue
            for label, keywords, check_point in _DORMANT_ASSET_RULES:
                if any(kw in raw_key for kw in keywords):
                    # 「有価証券」は「投資有価証券」と二重計上しないよう、既に投資有価証券で拾った行は除外
                    if label == "有価証券" and "投資有価証券" in raw_key:
                        break
                    prev = found.get(label, {"amount": 0, "check_point": check_point})
                    prev["amount"] += val
                    found[label] = prev
                    break

    items = [
        {"name": k, "amount": int(v["amount"]), "check_point": v["check_point"]}
        for k, v in found.items() if v["amount"] > 0
    ]
    items.sort(key=lambda x: x["amount"], reverse=True)
    total = sum(i["amount"] for i in items)

    ratio_to_cash = round(total / cash * 100, 1) if cash > 0 else None
    # 重要性：合計1,000万円以上 または 現預金の20%以上
    material = total >= 1000 or (cash > 0 and total >= cash * 0.20)

    return {
        "items": items,
        "total": total,
        "cash": int(cash),
        "ratio_to_cash": ratio_to_cash,
        "material": bool(items) and material,
    }


def _format_dormant_assets_for_prompt(info: dict, health: str = "") -> str:
    """寝ている資産をプロンプト用に整形。重要性が低ければ空文字（出さない）。"""
    if not info or not info.get("material"):
        return ""

    lines = [
        "",
        "【💤 寝ている資産＝固定化された資金（必ず読め）】",
        "決算書から検知した『現金化・回収・活用の余地がある資金』です。",
    ]
    for it in info["items"]:
        lines.append(f"  - {it['name']}: {it['amount']:,}万円 → 論点: {it['check_point']}")
    ratio = f"（現預金の{info['ratio_to_cash']}%相当）" if info.get("ratio_to_cash") is not None else ""
    lines.append(f"  ★ 合計: {info['total']:,}万円{ratio}")

    lines += [
        "",
        "🚨 扱い方（スタンス指示・必ず守れ）:",
        "- これは税理士が社長に説明できる『資金周り』の典型論点。事業の打ち手より優先度を上げてよい。",
        "- ただし**保障目的・事業上必要な資金は無理に課題化しない**（絶対ルール7と整合）。",
        "  あくまで『中身と方針を確認する余地』として扱う。",
        "- 各項目のうち**金額が大きく会社の状況に合うものだけ** observation_notes に1件記録せよ:",
        "  metric=項目名 / judgment=固定化の状況 / check_point=上記論点（回収・解約返戻金・税務論点）。",
    ]
    if health == "struggling":
        lines += [
            "- 🔴 資金繰りが苦しい局面: これらを『現金化候補・資金繰りクッション』として捉え、",
            "  growth_opportunities に addresses=\"現状機会: 寝た資産の活用\" で**現金化・回収の打ち手**を1件出してよい。",
        ]
    else:
        lines += [
            "- 健全/余剰局面: これらを『次の投資原資・回収整理』として捉え、状況に合えば",
            "  growth_opportunities に addresses=\"現状機会: 寝た資産の活用\" で提案してよい（必須ではない）。",
        ]
    lines.append("- fit しない（少額・保障目的明確・事業必須）なら無理に出さず、触れなくてよい。")

    return "\n".join(lines) + "\n"


def _format_partial_context_for_prompt(partial_ctx: dict, fd) -> str:
    """期中スナップショット分析のコンテキストをプロンプト用に整形。"""
    if not partial_ctx:
        return ""
    n = partial_ctx.get("n_months", 0)
    if n <= 0:
        return ""
    first = partial_ctx.get("first_period", "")
    last = partial_ctx.get("last_period", "")
    elapsed = partial_ctx.get("elapsed_ratio_pct", 0)
    ...
    lines = [
        "",
        "【🗓️ 期中スナップショット分析モード（必ず読め）】",
        f"このデータは **期中（FY進行中）の{n}ヶ月累計** です。期末ではないので、年次データではありません。",
        f"  - 累計期間: {first} 〜 {last}",
        f"  - 年度経過率: 約 {elapsed}%（{n}/12ヶ月）",
        f"  - 累計売上: {rev:,.0f}万円（{n}ヶ月分の累計）",
        f"  - 累計営業利益: {op:,.0f}万円",
        f"  - 現預金（期末時点）: {cash:,.0f}万円",
        "",
        "🚨 期中モードでの判断スタンス（必ず守れ）:",
        "1. 「年次データ」として扱わず、「期中n月累計」と明示すること",
        f"2. 数字を×12/n で年率換算するのは禁止（季節性無視で誤る）。\n   どうしても年率換算が要る場合は「単純按分すれば年率◯◯」と注釈付き",
        "3. 比較対象は **前年同期間（YoY）** が基本（下記参照）。前年通期との比較は使わない",
        "4. 進捗率は「年度の◯%が経過して売上は◯%達成」のように対比して見せる",
        "5. 結論は「このペースだと年度末は◯◯」より、「現時点でYoY◯%」を優先",
    ]

    yoy = partial_ctx.get("yoy_data")
    yoy_est = partial_ctx.get("yoy_estimate")
    if yoy:
        yoy_rev = yoy.get("revenue", 0)
        yoy_op = yoy.get("operating_profit", 0)
        yoy_gp = yoy.get("gross_profit", 0)
        rev_yoy_pct = round((rev - yoy_rev) / abs(yoy_rev) * 100, 1) if yoy_rev else None
        op_yoy_pct = round((op - yoy_op) / abs(yoy_op) * 100, 1) if yoy_op else None
        lines += [
            "",
            "【🔁 前年同期間（YoY）データ・実データ取得】",
            f"  - 前年同期間: {yoy.get('first_period','')} 〜 {yoy.get('last_period','')}",
            f"  - 前年同期間売上: {yoy_rev:,.0f}万円",
            f"  - 前年同期間営業利益: {yoy_op:,.0f}万円",
            f"  - 前年同期間粗利: {yoy_gp:,.0f}万円",
            f"  ★ 売上YoY: {rev_yoy_pct:+.1f}%" if rev_yoy_pct is not None else "  ★ 売上YoY: 計算不可",
            f"  ★ 営業利益YoY: {op_yoy_pct:+.1f}%" if op_yoy_pct is not None else "  ★ 営業利益YoY: 計算不可",
            "",
            "→ 上記YoYを掴みの核に置く。「前年同nヶ月と比べて売上◯%」を1行目に",
        ]
    elif yoy_est:
        lines += [
            "",
            "【🔁 前年同期間（YoY）データ・前年通期按分推定】",
            f"  ⚠️ 前年同期間の月次データが取れず、**前年通期 ÷ 12 × {n} の按分推定値** を使用",
            f"  ⚠️ 季節性を無視した推定なので、解釈は慎重に",
            f"  - 推定元: {yoy_est.get('source_period','')}",
            f"  - 推定前年同期間売上: {yoy_est.get('revenue',0):,.0f}万円（推定）",
            f"  - 推定前年同期間営業利益: {yoy_est.get('operating_profit',0):,.0f}万円（推定）",
            "→ 「（前年按分推定）」と注記しながらYoYを使う。データなし結論は避ける",
        ]
    else:
        lines += [
            "",
            "【🔁 前年同期間（YoY）データ・取得不可】",
            "  ❌ 前年同期間のデータが取得できない（前年データなし or 月次データ不足）",
            "→ YoY比較は使わず、**社内推移（前月比・初月比）** と **絶対水準（健全レンジ）** で評価する",
            "→ key_insight に「前年データ未取得のためYoY比較不可」と注記",
        ]
...
def generate_owner_pdf_content(result: dict, fd, client_name: str, period: str) -> dict:
    ...
    prompt = f"""あなたは税理士の先生に同席し、社長の **お金周り（資金繰り・節税・借入・キャッシュ）** を専門に見る資金アドバイザーです。
社長が一人で読んで自走できる、**ロジカル&エビデンスベース** の構造化レポートを書き出してください。
評価軸は資金周りの **絶対値と社内推移**（業界比較は精度が低いため使わない）。事業側の打ち手（新規事業・営業強化等）は主軸にせず、外部紹介か社長ヒアリングに回します。
...
def analyze_financials(fd: FinancialData, client_name: str, industry: str,
                       ...
                       reference_books: list = None,
                       raw_monthly_data: list = None) -> dict:
    """
    単年財務データを LLM で分析。
    ...
    """
    # ★ fdが年次系（annual / annual_aggregated）の場合、historical_data から
    # partial_aggregated を除外する。年次の trend/phase/growth_rates が n月累計と
    # 比較されて bogus な減少率を出すのを防ぐ。
    _fd_period_type = getattr(fd, "period_type", "") or ""
    if _fd_period_type in ("annual", "annual_aggregated") and historical_data:
        historical_data = [
            h for h in historical_data
            if getattr(h, "period_type", "") != "partial_aggregated"
        ]

    # ★ 月次データなら年次に集計してから分析する
    # 下流の全処理（_compute_idle_cash の ÷12、EBITDA、burn_rate、trend 等）が
    # 「年次データ前提」で書かれているため、月次レコードのまま流すと月額を年額扱いし、
    # 余剰キャッシュ判定が12倍誇張される等の誤発火が起きる。入口で集約して回避。
    #
    # 呼び出しルート2パターンに対応:
    #   (A) main.py: 既に build_effective_historical_data() を通っており、
    #       historical_data の同FY月次は AggregatedFinancial に置換済み → それを使う
    #   (B) 直接呼び出し: historical_data に生の月次が残っている → 自前で集計
    monthly_aggregation_meta = None
    if getattr(fd, "period_type", "") == "monthly":
        try:
            from .services.monthly_aggregator import (
                aggregate_monthly_to_fiscal_year,
                AggregatedFinancial,
            )
            fy = getattr(fd, "fiscal_year", "") or ""

            # (A) historical_data に同FYの annual_aggregated が既にあればそれを使う
            existing_agg = next(
                (f for f in (historical_data or [])
                 if getattr(f, "period_type", "") == "annual_aggregated"
                 and (getattr(f, "fiscal_year", "") or "") == fy),
                None,
            )
            if existing_agg is not None:
                fd = existing_agg
                monthly_aggregation_meta = {
                    "from_count": getattr(existing_agg, "_aggregated_from_count", 0),
                    "first_period": getattr(existing_agg, "_aggregated_first", ""),
                    "last_period": getattr(existing_agg, "_aggregated_last", ""),
                    "fiscal_year": fy,
                    "source": "prebuilt_by_caller",
                }
                print(f"[月次集計] {client_name} FY={fy} 呼び出し側の事前集計（{monthly_aggregation_meta['from_count']}ヶ月分）を使用", flush=True)
            else:
                # (B) 自前で集計。historical_data に fd が含まれる運用があるため、
                # id（同一性）で重複除去してから集計する。
                _fd_id = getattr(fd, "id", None)
                sibling_monthly = [
                    f for f in (historical_data or [])
                    if getattr(f, "period_type", "") == "monthly"
                    and (getattr(f, "fiscal_year", "") or "") == fy
                    and (getattr(f, "id", None) != _fd_id or _fd_id is None) and (f is not fd)
                ]
                _seen_ids = set()
                _unique = []
                for f in [fd] + sibling_monthly:
                    _key = getattr(f, "id", None)
                    if _key is None:
                        _key = id(f)
                    ...
                    _seen_ids.add(_key)
                    _unique.append(f)
                same_fy_monthly = _unique
                if len(same_fy_monthly) >= 3:
                    agg_data = aggregate_monthly_to_fiscal_year(same_fy_monthly)
                    if agg_data:
                        fd = AggregatedFinancial(agg_data)
                        monthly_aggregation_meta = {
                            "from_count": agg_data.get("_aggregated_from_count", 0),
                            "first_period": agg_data.get("_aggregated_first", ""),
                            "last_period": agg_data.get("_aggregated_last", ""),
                            "fiscal_year": fy,
                            "source": "computed_inline",
                        }
                        print(f"[月次集計] {client_name} FY={fy} {monthly_aggregation_meta['from_count']}ヶ月分を年次相当に集計", flush=True)
                else:
                    print(f"[月次集計スキップ] {client_name} FY={fy} 月次データが{len(same_fy_monthly)}件のみ（3件未満）— 月次のまま分析（÷12誇張リスクあり）", flush=True)
        except Exception as _e:
            # 集計失敗時は元の fd を使用（既存挙動を維持）
            print(f"[月次集計エラー] {_e} — 元の月次データのまま分析", flush=True)

    # ★ 期中スナップショット（partial_aggregated）モードの判定とYoY前年同期間取得
    # period_type が partial_aggregated の場合、
    # ・現在 n ヶ月累計のデータで分析（÷n は _compute_idle_cash で対応済み）
    # ・前年同 n ヶ月の累計（YoY）を取得してプロンプトに埋め込む
    # ・進捗率（年度経過率 vs 売上達成率）を計算
    partial_context = None
    if getattr(fd, "period_type", "") == "partial_aggregated":
        try:
            from .services.monthly_aggregator import get_yoy_period_data
            n_months = getattr(fd, "_partial_n_months", 0) or 0
            first_period = getattr(fd, "_aggregated_first", "") or ""
            last_period = getattr(fd, "_aggregated_last", "") or ""
            # YoY 取得用に、historical_data 中の生月次データ + 現在 fd を all_fds として渡す
            # ただし、main.py は build_effective_historical_data 経由で集計値だけ渡してくる可能性あり
            # → そのケースでも前年annual_aggregatedから推定で取れる
            target_partial_dict = {
                "_aggregated_first": first_period,
                "_aggregated_last": last_period,
                "_aggregated_from_count": n_months,
            }
            # ★ YoY検索用データの優先順位:
            #   1. raw_monthly_data（main.py から明示的に渡された生月次）
            #   2. historical_data の中の生月次（直接呼び出しルート）
            all_fds_for_yoy = list(raw_monthly_data or []) + list(historical_data or [])
            yoy_data = get_yoy_period_data(target_partial_dict, all_fds_for_yoy)
            # 2. 取れない場合、前年annual_aggregated or annual を ÷12*n で按分推定
            yoy_estimate_basis = None
            if not yoy_data:
                import re as _re_yoy
                _year = 0
                _ym = _re_yoy.search(r"(\d{4})", getattr(fd, "fiscal_year", "") or "")
                if _ym:
                    _year = int(_ym.group(1))
                if _year > 0:
                    prev_year_str = str(_year - 1)
                    prev_year_fd = next(
                        (h for h in all_fds_for_yoy
                         if prev_year_str in (getattr(h, "fiscal_year", "") or getattr(h, "period", ""))
                         and getattr(h, "period_type", "") in ("annual", "annual_aggregated")),
                        None,
                    )
                    if prev_year_fd and n_months > 0:
                        # ÷12 × n で按分（季節性無視の推定値）
                        _factor = n_months / 12.0
                        yoy_estimate_basis = {
                            "source_period": getattr(prev_year_fd, "period", ""),
                            "factor": _factor,
                            "revenue": (getattr(prev_year_fd, "revenue", 0) or 0) * _factor,
                            "gross_profit": (getattr(prev_year_fd, "gross_profit", 0) or 0) * _factor,
                            "operating_profit": (getattr(prev_year_fd, "operating_profit", 0) or 0) * _factor,
                            "selling_expenses": (getattr(prev_year_fd, "selling_expenses", 0) or 0) * _factor,
                        }

            # 進捗率の計算（決算月から年度経過率を推定）
            elapsed_ratio = None
            if n_months > 0:
                elapsed_ratio = round(n_months / 12.0 * 100, 1)  # % 経過

            partial_context = {
                "n_months": n_months,
                "first_period": first_period,
                "last_period": last_period,
                "elapsed_ratio_pct": elapsed_ratio,
                "yoy_data": {
                    "available": yoy_data is not None,
                    "first_period": (yoy_data or {}).get("_aggregated_first", ""),
                    "last_period": (yoy_data or {}).get("_aggregated_last", ""),
                    "revenue": (yoy_data or {}).get("revenue", 0),
                    "gross_profit": (yoy_data or {}).get("gross_profit", 0),
                    "operating_profit": (yoy_data or {}).get("operating_profit", 0),
                    "selling_expenses": (yoy_data or {}).get("selling_expenses", 0),
                } if yoy_data else None,
                "yoy_estimate": yoy_estimate_basis,
            }
            print(f"[期中分析] {client_name} 期中{n_months}ヶ月 ({first_period}〜{last_period}) / YoY={'実データ取得' if yoy_data else ('前年按分推定' if yoy_estimate_basis else 'データなし')}", flush=True)
        except Exception as _e:
            print(f"[期中分析エラー] {_e}", flush=True)

    # historical_data の整合化（集計が走った場合のみ）:
    # 1) 同FYの生の月次行を除去（trend ダブルカウント防止）
    # 2) (A)で fd に採用した既存 AggregatedFinancial も除去（fdと history の両方に同じ期が入る重複防止）
    # 3) 集計後の fd を history に追加（下流の前期検出 `h.id == fd.id` 走査が止まるように）
    if monthly_aggregation_meta and historical_data is not None:
        _fy = monthly_aggregation_meta["fiscal_year"]
        _fd_obj_id = id(fd)
        historical_data = [
            f for f in historical_data
            if not (
                (getattr(f, "period_type", "") == "monthly"
                 and (getattr(f, "fiscal_year", "") or "") == _fy)
                or (getattr(f, "period_type", "") == "annual_aggregated"
                    and (getattr(f, "fiscal_year", "") or "") == _fy)
                or id(f) == _fd_obj_id  # fd 自身が混入してたら除去（次行で改めて追加）
            )
        ]
        # 集計後の fd を history に追加（前期検出ロジック互換のため）
        historical_data = historical_data + [fd]
    ...
    # 期中（partial_aggregated）の場合は n月累計 vs 12ヶ月 の比較になって誤解を生むのでスキップ
    cost_delta_text = ""
    _is_partial_for_delta = getattr(fd, "period_type", "") == "partial_aggregated"
    if (not _is_partial_for_delta) and historical_data and len(historical_data) >= 2:
        try:
            sorted_hd = sorted(historical_data, key=lambda x: x.period or "")
            curr_fd = None
            prev_fd = None
    ...
    # ★ 期中モード（partial_aggregated）では、historical_text/trend_metrics の
    # 前期比計算は「期間長の違う期同士」になるため意味のある比較ができない。
    # → 期中モードでは年次trendをスキップし、YoY比較 (partial_context.yoy_data) を主に使う
    _skip_annual_trend = getattr(fd, "period_type", "") == "partial_aggregated"
    if _skip_annual_trend:
        historical_text = "\n【⏰ 期中分析モード】通常の前期比トレンドはスキップ。YoY（前年同期間）比較を使う（partial_context.yoy_data 参照）。\n"
        # trend_metrics は最低限のラベルだけセット（グラフ描画用）
        if historical_data:
            # 年次トレンド用：期中(partial)と月次(monthly)は除外（月次は別の年度別重ねグラフで表示）
            _hd_no_partial = [
                h for h in historical_data
                if getattr(h, "period_type", "") not in ("partial_aggregated", "monthly")
            ]
            sorted_hd = sorted(_hd_no_partial, key=lambda x: x.period or "")
            ...
            trend_metrics["_partial_mode"] = True
    elif historical_data and len(historical_data) > 1:
        # period 昇順で並べる（期中・月次は除外して年次のみで trend を作る。
        # 月次データは別途「月次推移グラフ（年度別重ね）」で表示するため、ここに混ぜると
        # 期比較グラフ（売上vs費用構造/BS推移/時系列トレンド/現預金推移）が月次点で潰れる）
        sorted_hd = sorted(
            [h for h in historical_data
             if getattr(h, "period_type", "") not in ("partial_aggregated", "monthly")],
            key=lambda x: x.period or "",
        )
        if not sorted_hd:
            sorted_hd = sorted(historical_data, key=lambda x: x.period or "")
    ...
    # 経営フェーズ判定（3年トレンド分析）
    # ★ 期中モード（partial_aggregated）では、n月累計を前期年次と比較すると
    # 売上が低く出て「再建期」「停滞期」と誤判定する。期中時は phase 判定をスキップ。
    if getattr(fd, "period_type", "") == "partial_aggregated":
        phase_info = {"phase": "unknown", "label": "期中分析モード（年次フェーズ判定スキップ）", "evidence": []}
    else:
    ...
    # 💤 寝ている資産（保険積立金・貸付金・有価証券等）の検知（決定的・状況に合えば提案）
    dormant_info = _compute_dormant_assets(fd)
    dormant_text = _format_dormant_assets_for_prompt(dormant_info, health=state.get("health", ""))

    # 🗓️ 期中スナップショット（partial_aggregated）の場合は YoY比較・進捗率を埋め込む
    partial_text = _format_partial_context_for_prompt(partial_context, fd) if partial_context else ""

    # 📚 税理士が参照を選んだ書籍の知見（書籍ナレッジDB）
    # 著作権配慮: 本文をそのまま転載せず、要点を自分の言葉で再表現させる
    ref_books_text = ""
    if reference_books:
        _parts = [
            "",
            "【税理士が参照を選んだ書籍の知見】",
            "※以下の考え方・判断軸を踏まえて助言を組み立てること。",
            "※ただし本文をそのまま転載せず、要点を自分の言葉で再表現すること（著作権配慮）。",
        ]
        _added = 0
        _total = 0
        _MAX_PER_BOOK = 6000      # 1冊あたりの上限（濃い判断軸DBを許容）
        _MAX_TOTAL = 20000        # 全書籍合計の上限（プロンプト肥大・コスト/コンテキスト超過を防ぐ）
        for _b in reference_books:
            if _total >= _MAX_TOTAL:
                break
            _content = (_b.get("content", "") or "").strip()[:_MAX_PER_BOOK]
            if not _content:
                continue
            # 合計上限を超える分は切り詰め
            _remain = _MAX_TOTAL - _total
            if len(_content) > _remain:
                _content = _content[:_remain]
            if not _content:
                break
            _title = _b.get("title", "") or ""
            _author = _b.get("author", "") or ""
            _parts.append(f"■『{_title}』{_author}\n{_content}")
            _total += len(_content)
            _added += 1
        ref_books_text = "\n".join(_parts) if _added > 0 else ""
    ...
    prompt = f"""あなたは税理士の先生に同席し、社長の「お金周り（資金繰り・節税・借入・キャッシュ）」を専門に見る資金アドバイザーです。
提案の軸は必ず資金周りに置きます。税理士の先生が自分の言葉で社長に説明できる土俵＝お金の話だからです。
新規事業・FC加盟・M&A・営業強化などの「事業側の打ち手」は、税理士が踏み込みにくい領域なので主軸にせず、外部パートナー紹介（executor=referral_partner）か社長へのヒアリング項目に回します。
...
11. **🎯 課題 → 分析 → 提案 の整合性（最重要・出力の質はここで決まる）**
    すべての提案（prioritized_problems.solutions, growth_opportunities）は、必ず以下の論理チェーンで繋ぐ:

    **事実（fact）→ 仮説（hypothesis）→ 打ち手（solution / opportunity）→ 効果（impact）**

    各 growth_opportunity / solution には必須:
    - `addresses`: この提案が解く対象を明示する文字列。以下のいずれか:
      ・「課題rank-X」（prioritized_problems の rank=X を解決）
      ・「観察ノート: XX」（observation_notes の指摘に対応）
      ・「現状機会: 余剰キャッシュ活用」「現状機会: 既存投資の回収最大化」など
    - `why_now`: なぜ今このタイミングで取り組むべきか（1文）
    - `expected_change`: 実行前後の数値変化（「営業利益率3.3%→7%」「現預金月+750万→+900万」等）

    🚨 整合性NGパターン（絶対禁止）:
    ・課題ゼロなのに「即時償却で節税」だけ単独で出す → 何の課題を解いてるか不明
    ・「営業利益悪化」を rank=1 課題化したのに、提案が「M&A」「不動産投資」だけ → ズレてる
    ・「余剰キャッシュ寝てる」と書いたのに、提案に余剰活用シナリオがない → 矛盾
    ・「投資期で減価償却急増」と書いたのに、提案に「投資効果検証」がない → 抜け

    出力検証: 各 prioritized_problem に対し solutions が「その課題を解く方向」になっているか、
    各 growth_opportunity に対し addresses で対応する観察/機会が紐づいているかをチェックせよ。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{{
  "key_insight": "<1行の最重要ポイント。核心を一撃で。🚨掴み＝お金周りの数字で危機感or機会を一言で言い切る（現預金の減少ペース／寝ている現金／節税できる額など、必ず数字を入れる）。例『本業は黒字。ただ現金は3年で1.3億→6,600万円、毎月193万円のペースで減っています』>",
    ...
    {{
      ...
      "solutions": [
        {{
          ...
          "addresses": "<🚨 必須：このsolutionが解く対象を親課題と紐付けて明示。原則『課題rank-X』（属する prioritized_problems の rank と一致させる）。複合の場合『課題rank-1 + 観察ノート: 在庫回転日数』等。曖昧な『全体改善』はNG>",
          "why_now": "<🚨 必須：なぜ今このタイミングで取り組むべきか1文。決算前/黒字着地/キャッシュ余力等の文脈と紐付ける>",
          "expected_change": "<🚨 必須：実行前後の数値変化。Before→After形式で。例『売上依存度77%→50%（▲27pt）』『月次キャッシュ▲30万→+20万（+50万/月）』。impact_min/maxとは別に、KPI観点での変化を書く>"
        }}
      ],
  ...
  "owner_message": "<社長向け1-2文。数字が苦手な経営者が30秒で判断できる言葉で。専門用語禁止（幼児語ではなく、経営者語）。**不安を煽る表現禁止**（『危険』『底をつく』『破綻』→ 『気になる』『少し心配な』『余裕が薄くなってきた』に置換）。🚨必ずお金周りの数字から入る（現預金・毎月の増減ペース・あと何ヶ月の余力／余剰キャッシュ・節税できる額のいずれか）。**事実+所感+問いかけ**の順で。例：『本業は黒字ですが、現金が3年で1.3億→6,600万円に減ってきています。借入の返済が月193万円あるためです。このペースだと余裕が薄くなっていく感じがあります。』>",
  ...
  "segments": [
    {{
      "name": "<事業部・セグメント名。例『お茶卸売』『EC直販』『飲食店向け』>",
      "revenue": <万円。この事業の売上>,
      "revenue_share": "<全体に占める%。例『40%』>",
      "gross_margin": "<粗利率。分からなければ空文字>",
      "trend": "<前期比トレンド。『拡大中』『微減』『横ばい』等。データなければ空文字>",
      "key_facts": "<この事業の重要な事実1-2文。社長語＋カッコ説明>",
      "issues": ["<この事業固有の課題>"],
      "opportunities": ["<この事業の成長機会>"]
    }}
  ],
  "growth_opportunities": [
    {{
      "rank": <1-7の整数。🚨資金周り優先ルール（最重要）：rank1-2は必ず「お金周りコア層」＝tax_optimization（節税）/ investment（余剰キャッシュ・寝た資産の運用）/ cfo_outsource（資金管理の伴走）から選ぶ。税理士が自分の言葉で説明できる土俵だから。事業側カテゴリ（new_business/franchise/m_and_a/ipo）をrank1-2に置くのは原則禁止。やむを得ず置く場合は必ず executor=referral_partner（税理士の土俵外＝外部紹介）にすること。※資金繰り改善・借入最適化そのものは growth_opportunities ではなく prioritized_problems.solutions（category=finance）側で扱う>,
      "category": "<tax_optimization/employee_return/investment/new_business/franchise/m_and_a/ipo/regulation/cfo_outsource のいずれか。succession（事業承継）は hearing_sheet で社長年齢・後継者を確認するまで提案禁止。🚨お金周りコア層（tax_optimization/investment/cfo_outsource）を優先し、new_business/franchise/m_and_a/ipo等の事業側は周辺扱い（referral_partner経由）>",
      ...
      "addresses": "<🚨 必須：この提案が解く対象を明示。次のいずれかの形式で書く：『課題rank-1』『課題rank-2』（prioritized_problems の特定rankを解決）/『観察ノート: 売掛回収日数』（observation_notes の指摘に対応）/『現状機会: 余剰キャッシュ活用』『現状機会: 既存投資の回収最大化』『現状機会: 黒字節税』等。空文字や曖昧な文字列（『全体改善』等）はNG>",
      "why_now": "<🚨 必須：なぜ今このタイミングで取り組むべきか1文。例『黒字着地で課税所得2,400万円が見えている今期中でないと節税効果が消える』『余剰キャッシュ4,343万円が無利息で寝ており機会損失が継続している』>",
      "expected_change": "<🚨 必須：実行前後の数値変化を明示。例『営業利益率3.3%→7%（+3.7pt）』『現預金月次キャッシュ寄与+0万円→+50万円（年+600万）』『法人税額▲792万円』。曖昧表現（『改善』『向上』）はNG、必ず数字Before→Afterで>"
    }}
  ],
...
【🏢 事業部別分析ルール】
business_details または breakdown.revenue_detail に事業部別・セグメント別の数字が含まれる場合、
**segments 配列に事業部ごとの分析を出力**。1事業のみなら segments=[]（空）でOK。

- 各事業の売上構成比・粗利率・トレンドを社長語で（カッコ説明付き）
- 各事業固有の課題と機会を別個に
- 売上構成比 10% 未満の小規模事業は省略可
- prioritized_problems / growth_opportunities は会社全体としての分析（事業部横断）

例（山口茶業の場合）:
- お茶卸売（売上60%）/ お茶小売（25%）/ EC直販（15%）— 各事業の課題と機会を分けて出す
- 「卸売は粗利率18%で安定、EC直販は粗利率35%だが規模が小さい」「EC直販に追加投資で粗利貢献を狙う」等
...
- 桁ミス検証: 出力した数字が入力と桁照合できるか必ず確認"""
    ...
    result["dormant_assets"] = dormant_info  # 💤 寝ている資産（固定化資金）の検知結果
    if monthly_aggregation_meta:
        result["monthly_aggregation"] = monthly_aggregation_meta  # 月次→年次集計のメタ情報（UI/監査用）
    if partial_context:
        result["partial_context"] = partial_context  # 🗓️ 期中スナップショット分析のコンテキスト
    ...
    if historical_data:
        ...
        for h in sorted_hd_for_prev:
            ...
            prev_period_data = h
    growth_rates = {}
    growth_rates_yoy = {}  # 期中専用: YoY（前年同期間比）データ
    # ★ 期中スナップショット時は通常の前期比計算をスキップ
    # 期中 n月累計を前年通期と比較すると数字が崩壊するため。
    # YoY値は growth_rates_yoy に別キーで保存し、既存の growth_rates（=前期比）と混同しないように。
    # こうしないと analysis.html が「前期比」ラベルで YoY値を表示してしまう。
    _is_partial = getattr(fd, "period_type", "") == "partial_aggregated"
    if _is_partial:
        growth_rates["_partial_mode"] = True
        growth_rates["_note"] = "期中分析モード：通常の前期比はスキップ。YoY は growth_rates_yoy 参照"
        # YoY実データがあれば growth_rates_yoy に保存
        _yoy = (partial_context or {}).get("yoy_data") if partial_context else None
        if _yoy:
            yr = _yoy.get("revenue", 0); yo = _yoy.get("operating_profit", 0); yg = _yoy.get("gross_profit", 0)
            if yr:
                growth_rates_yoy["revenue"] = round((fd.revenue - yr) / abs(yr) * 100, 1)
            if yo:
                growth_rates_yoy["operating_profit"] = round((fd.operating_profit - yo) / abs(yo) * 100, 1)
            if yg:
                growth_rates_yoy["gross_profit"] = round((fd.gross_profit - yg) / abs(yg) * 100, 1)
            growth_rates_yoy["_source"] = "real_data"
            growth_rates_yoy["_label"] = "前年同期間比 (YoY)"
        elif partial_context and partial_context.get("yoy_estimate"):
            _est = partial_context["yoy_estimate"]
            er = _est.get("revenue", 0); eo = _est.get("operating_profit", 0); eg = _est.get("gross_profit", 0)
            if er:
                growth_rates_yoy["revenue"] = round((fd.revenue - er) / abs(er) * 100, 1)
            if eo:
                growth_rates_yoy["operating_profit"] = round((fd.operating_profit - eo) / abs(eo) * 100, 1)
            if eg:
                growth_rates_yoy["gross_profit"] = round((fd.gross_profit - eg) / abs(eg) * 100, 1)
            growth_rates_yoy["_source"] = "annual_prorated_estimate"
            growth_rates_yoy["_label"] = "前年同期間比 (YoY・按分推定)"
        else:
            growth_rates_yoy["_source"] = "unavailable"
            growth_rates_yoy["_label"] = "YoYデータなし"
    elif prev_period_data:
    ...
    if growth_rates_yoy:
        result["growth_rates_yoy"] = growth_rates_yoy

    # 健全性スコアを Python 計算で上書き（揺らぎ防止・ロジック固定）
    # historical_data も渡して「数字の動きの異常」もペナルティに反映
    # ★ 期中（partial_aggregated）の場合、compute_health_score 内の効率指標
    # （receivables/revenue×365 等）が n月累計で歪むので、年率換算した PL を持つ
    # 軽量ラッパで渡す
    _score_fd = fd
    if _fd_period_type == "partial_aggregated":
        _n = max(getattr(fd, "_partial_n_months", 0) or 1, 1)
        _factor = 12.0 / _n
        class _AnnualizedFD:
            pass
        _score_fd = _AnnualizedFD()
        # 全属性をコピー
        for attr in ("id", "period_type", "fiscal_year", "period", "client_id",
                     "cash", "receivables", "inventory", "total_assets",
                     "current_assets", "total_liabilities", "current_liabilities",
                     "interest_bearing_debt", "equity", "employees",
                     "breakdown_json"):
            setattr(_score_fd, attr, getattr(fd, attr, None))
        # PL 系のみ年率換算
        for pl_attr in ("revenue", "cost_of_sales", "gross_profit",
                        "selling_expenses", "operating_profit",
                        "ordinary_profit", "net_profit"):
            setattr(_score_fd, pl_attr, (getattr(fd, pl_attr, 0) or 0) * _factor)
    health = compute_health_score(_score_fd, benchmark=benchmark, ebitda=ebitda,
